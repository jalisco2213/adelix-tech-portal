...
const exportToZIP = async () => {
  ...
  for (let i = 0; i < shields.value.length; i++) {
    const element = document.querySelectorAll('.UT20-info')[i];
    if (element) {
      ...
      zip.file(`ADL-UT20_${shields.value[i].serial}.png`, imageBlob);
    }
  }

  const pdfBlob = pdf.output('blob');
  zip.file(`ADL-UT20_${serial.value}.pdf`, pdfBlob);

  const cdrBlob = pdf.output('blob');
  zip.file(`ADL-UT20_${serial.value}.cdr`, cdrBlob);

  const content = await zip.generateAsync({ type: 'blob' });
  const link = document.createElement('a');
  link.href = URL.createObjectURL(content);
  link.download = `ADL-UT20_${serial.value}.zip`;
  link.click();
};
...
<template>
  <h1 class="device-header">Ультразвуковой толщиномер <br> ADL-UT20</h1>
  ...
  <div class="shields-container">
    <div class="UT20-container" v-for="(shield, index) in shields" :key="shield.id">
      <div class="UT20-info">
        <div class="UT20-content">
          <div class="UT20-header">
            ...
            <h1 v-if="!country" style="padding: 7px 0 10px; line-height: 1;">
              ...
              <br>
              <span style="font-size: 25px; letter-spacing: 1px; font-weight: 800;">ADL-UT20</span>
            </h1>
          ...
          <div class="UT20-center">
            ...
            <div style="margin-top: 5px;" class="UT20-part">
